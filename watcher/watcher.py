#!/usr/bin/env python3
"""
Log watcher - tails nginx JSON access log, computes sliding-window error rate,
detects pool failovers, and posts alerts to Slack (or writes to outbox for local testing).

Env vars:
- LOG_PATH (default /var/log/nginx/access.log)
- SLACK_WEBHOOK_URL
- ERROR_RATE_THRESHOLD (percent, default 2)
- WINDOW_SIZE (default 200)
- ALERT_COOLDOWN_SEC (default 300)
- MAINTENANCE_MODE (0|1)
- OUTBOX_PATH (default /watcher/outbox.log)
"""
import os
import sys
import time
...
def send_alert(alert_type, title, body):
    ...
    if ts - last < ALERT_COOLDOWN_SEC:
        logger.info('Skipping alert %s: cooldown', alert_type)
        return False
    if MAINTENANCE_MODE:
...
def parse_line(line):
    try:
        ...
        logger.debug('Non-JSON line, skipping: %s', line.strip())
        return None
    return data
...
def tail_file(path):
    while True:
        try:
            ...
            logger.error('Log file not found: %s. Retrying in 1s.', path)
            time.sleep(1)
        except Exception:
            logger.exception('Unexpected error in tail_file; retrying in 1s')
            time.sleep(1)
